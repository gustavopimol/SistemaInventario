...
<div class="card ">
	<div class="card-header">
	  <h2 class="font-weight-bold">EDICION DE ALMACEN</h2>
	</div>
	<!-- /.card-header -->
	<div class="card-body">
		<div class="col-8 m-auto borde mb-3">
			<div class="form-group modal-content bg-dark p-5 mb-4">
			<form method="POST" action="{{ action('AlmacenController@update',$almacen->idalmacen) }}">
				@csrf
				{{ method_field('PUT') }}
			{{-- <input name="_method" type="hidden" value="PATCH"> --}}
			<label for="Nombre" class="text-success font-weight-bold">ALMACEN</label>
			<input type="text" name="almacen" class="form-control text-white font-weight-bold bg-dark border-success mb-1" value="{{ $almacen->almacen}}" required="">
			<label for="Direccion" class="text-success font-weight-bold">DESCRIPCION</label>
			<input type="text" name="descripcion" class="form-control text-white font-weight-bold bg-dark border-success mb-1" value="{{ $almacen->descripcion}}" required="">
